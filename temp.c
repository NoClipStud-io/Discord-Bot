...
int y_coordinate_from_compressed(mpz_t y, mpz_t x, const char *prefix,
                                 mpz_t p) {
  ...
  if ((mpz_tstbit(beta, 0) == 0 && prefix[1] == '2') ||
      (mpz_tstbit(beta, 0) == 1 && prefix[1] == '3')) {
    mpz_set(y, beta);
  } else {
    mpz_sub(y, p, beta);
  }
...
void show_progress(uint64_t current_step, mpz_t max_k, mpz_t key_range,
  ...
  if (elapsed_sec >= 1.0) {
    double current_steps_per_sec =
        (double)(current_step - last_step_count) / elapsed_sec;
    ...
    printf("\rSteps: %llu %.2f %s Key Range: %d bits",
           (unsigned long long)current_step, current_steps_per_sec,
           steps_per_second_unit, bits);
    fflush(stdout);
...
void brent_algorithm(ec_point_t *start_point, ec_point_t *public_key, mpz_t p,
                     volatile uint64_t *found_private_key) {
...
void *thread_function(void *arg) {
  ...
  while (mpz_cmp(current_k, thread_args->end_k) < 0 && !found_collision) {
    ...
    show_progress(current_step, thread_args->end_k, thread_args->end_k,
                  &current_time);
...
int main(int argc, char *argv[]) {
  ...
  if (!y_coordinate_from_compressed(public_key_a.y, public_key_a.x, prefix,
                                    p)) {
    fprintf(stderr,
